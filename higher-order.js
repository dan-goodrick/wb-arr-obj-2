...
/*
  ...
  (element, index, wholeArray)=>{}    Arrow Form
*/
const map2 = function(arr, callback){
  output = []
...
const filter2 = function(arr, callback){
  output = []
  for (let i=0; i<arr.length; i++){
    if (callback(arr[i])) {
      output.push(arr[i])
...
//Code Here
let postTaxPrices2 = filter2([1,2,3], function(val){
  return val%2===0
})
let postTaxPrices = prices.map(function(val){
  return val * 1.07
})
...
*/
const reduce2 = function(arr, accum, callback, ){
for (let i=0; i < arr.length; i++){
  accum += callback(arr[i])
}
return accum
}
//Code Here
let totalPopulation2 = reduce2(populations, accum=0, function(val){
  return accum + val
})
let totalPopulation = populations.reduce(function(val, accum){
  return accum+val
}, 0)
...
let bobsTotal = purchases.reduce(function(accum, obj){
  if (obj.owner === 'Bob'){
  accum += obj.price}
  return accum
}, 0)
